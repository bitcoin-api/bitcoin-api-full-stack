...
| bitcoin_api_exchangeEmailDeliveryResults TODO: 🚧👷‍♂️👷‍♀️🏗 update documentation - is for exchange only | email (string) | creationDate (number) | 
...
Q-Locks run the operations their locking in series as opposed to running concurrently in parallel. If an action is in a Q-Lock and another action with the same Q-Lock attempts to be performed, it will have to wait until the first action has finished in the Q-Lock. A Q-Lock is identified in Bitcoin-Api in the following format `{type}:{id}`. An example of this would be the Q-Lock used in the  `withdraws:user_id` POST /withdraws endpoint. This endpoint starts the withdraw process for a user attempting to make a withdraw. What having the Q-Lock on this withdraw operation means is that for an individual user, a second withdraw operation can only occure after the first withdraw has finished, this prevents double spends.👨🏿‍🔬
