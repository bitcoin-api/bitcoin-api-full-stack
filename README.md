...
The "frontend" webapp set up instructions and code for an exchange and casino that connects to your built Bitcoin-Api backend are provided. A processed version of the frontend code is sent to users' devices which implies it's publicly accessible. Front end users retrieve data (such as Bitcoin balance data) from the backend API that they're authorized to access.
